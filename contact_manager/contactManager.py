...
class ContactMgr:
	...
	def login(self):
		...
		query = ("""SELECT username FROM logindetails WHERE username = %s and password = %s""")
		data = (user, pswrd)
		...
		if not data:
			...
			cursor.execute('SELECT profile from logindetails WHERE username = %s and password = %s', (user, hashed_password))
			data = cursor.fetchone()
